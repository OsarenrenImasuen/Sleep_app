...
let state = {
  ...
  lastClockInDate: null,  // "YYYY-MM-DD" of last clock-in — ADD THIS LINE
...
async function askAI() {
  ...
  const thinkingBubble = appendChatBubble("ai", "Thinking…");
 
  const payload = {
    message: question,
    sleepLog: state.sleepLog,      // last 60 nights: date, bedtime, wake, hoursSlept
    summary: {
      nights: state.nights,
      totalHours: state.totalHours,
      avgHours: state.nights ? Math.round((state.totalHours / state.nights) * 10) / 10 : 0,
      level: getLevelInfo(state.xp).current.title
    }
  };
 
  try {
    const res = await fetch("/api/sleep-chat", {
      method: "POST",
    ...
    thinkingBubble.textContent = "Sorry, I couldn't reach the AI coach right now. (" + err.message + ")";
  }
}
...
function updateLiveClock() {
  ...
  document.getElementById("liveClockTz").textContent =
...
function updateClockInButton() {
  ...
  if (!btn) return;
  const alreadyClockedIn = state.lastClockInDate === getTodayDateString();
  btn.disabled = alreadyClockedIn;
  btn.textContent = alreadyClockedIn ? "✓ Clocked In" : "🕐 Clock In Now";
}

function clockIn() {
  const today = getTodayDateString();
  if (state.lastClockInDate === today) {
    showToast("You've already clocked in today!");
    return;
  }
  const now = new Date();
  const hh = String(now.getHours()).padStart(2,"0");
  const mm = String(now.getMinutes()).padStart(2,"0");
  document.getElementById("sleepStart").value = hh + ":" + mm;
  state.lastClockInDate = today;
  save();
  updateClockInButton();
  showToast("🕐 Clocked in at " + hh + ":" + mm);
}
...
function renderStore() {
  ...
  STORE_ITEMS.forEach(function(item) {
    ...
    let permanentLine;
    if (isPermanentOwned == true){
      permanentLine = '<p style="font-size:.78rem;color:#2a7a2a;font-weight:500;">✓ Purchased — active permanently</p>'
    } else{
      if (item.permanent){
         permanentLine = '<p style="font-size:.78rem;color:#2a7a2a;font-weight:500;">One-time permanent upgrade</p>'
      }
      else{
        permanentLine = '<p style="font-size:.78rem;color:#3a5a7a;">You own: <strong>' + owned + '</strong></p>'
      }
    }
    let footerRight 
    if (isPermanentOwned == true){
      footerRight = '<span class="owned-badge" style="display:inline-block;">Owned</span>'
    } else {
      footerRight = '<button class="btn btn-gold btn-sm" onclick="buyItem(\'' + item.id + '\')">Buy</button>'
    }
...
function updateAllUI() {
  ...
  const avg = "—";
  if (state.nights >0){
    avg = (state.totalHours / state.nights).toFixed(1) + "h";
  }
